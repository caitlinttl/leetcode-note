...
public class java_note { 
    ...
    public static void name_data_type() {
        ...
        // 繼承 v.s. 接口
        // extends: "繼承" 之間的關係是is-a，也就意味著子類是父類的一個子集。
        // implements: "接口" 的類和接口之間的關係是has-a，意味著具體類擁有接口的一些功能。


        /* 
        @overriding(覆寫) 同名同式(或覆載)
        Manager class重新寫一個setPay()去蓋過Employee原先的setPay()
        Override父類別的方法時，方法的名稱、參數、傳回值都要與父類別的一致。

        overloading (同名異式)
        Overloading是指同一件事情，可能有多種方式去完成，依照參數的形式與數量不同，可以做出不一樣的結果。
        使用"相同的方法名"，來處理不同的參數，比如下面這些方法。
        ...
        */
        ...
        我們必須把ArrayList變成一種模板：ArrayList<T>，其中的T即為各種類別的代替品。T 即為可以代
        private T[] array;
    ...
    public static void name_condition() {
        ...
        java_note a = new java_note("name");
    ...
    public static void name_array() {
        ...
        System.out.println(names.equals("123")); 
    ...
    public static void name_String() {
        ...
        System.out.println(message.charAt(1)); // 回傳指定索引的字元
        ...
        System.out.println(message.substring(1, 2)); // 不含endIndex
